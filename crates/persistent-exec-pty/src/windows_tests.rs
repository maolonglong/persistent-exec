...
async fn assert_terminate_kills_descendant(
    ...
    // Exercise descendants created immediately after the contained root starts.
    let code = format!(
...
async fn assert_normal_exit_cleans_up_descendant(
    backend: &str,
    python: &str,
    env: &HashMap<String, String>,
) -> anyhow::Result<()> {
    ...
    assert!(!survived, "{backend} descendant survived session cleanup");
    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn terminate_kills_descendants_for_pipe_and_conpty() -> anyhow::Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn normal_exit_cleans_up_descendants_for_pipe_and_conpty() -> anyhow::Result<()> {
    let Some(python) = find_python() else {
        eprintln!("python not found; skipping Windows process-tree natural-exit test");
        return Ok(());
    };
    let env: HashMap<String, String> = std::env::vars().collect();
    assert_normal_exit_cleans_up_descendant("pipe", &python, &env).await?;
    assert_normal_exit_cleans_up_descendant("ConPTY", &python, &env).await
}
