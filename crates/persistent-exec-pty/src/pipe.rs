...
/// On Windows, the process starts suspended and is assigned to a Job Object
/// before it is resumed, when the host permits nested job containment.
async fn spawn_process_with_stdin_mode(
    ...
    let windows_terminator = {
        ...
        match job {
            Ok(job) if job.assign_and_resume_process(pid)? => WindowsChildTerminator::Job(job),
            Ok(_) => WindowsChildTerminator::Process(pid),
            Err(err) => {
                log::warn!(
