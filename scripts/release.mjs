...
if (!["patch", "minor"].includes(releaseType)) {
...
if (
  ...
  throw new Error(
    "all package manifests must have the same version before release",
  );
}
...
for (const [path, manifest] of manifests) {
  ...
  for (const section of ["dependencies", "optionalDependencies"]) {
    for (const name of Object.keys(manifest[section] ?? {})) {
      if (name.startsWith("persistent-exec-"))
...
for (const crate of [
  ...
  if (!pattern.test(cargoLock))
    throw new Error(`failed to update ${crate} in Cargo.lock`);
...
writeFileSync(
  changelogPath,
  changelog.replace("## [Unreleased]", `## [${version}] - ${date}`),
);
